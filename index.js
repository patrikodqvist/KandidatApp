/**
 * Created by philipclaesson on 2017-03-01.
 */
...
var main =  function() {
    ...
    this.readFile = function () {
        ...
        Papa.parse(file, { //csv --> json
            ...
            dynamicTyping: true,
            encoding : "UTF-8",
            complete: function callCalc(jsonFile){
                //calc(jsonFile,settings); //Calls calc
                globalJsonFile = jsonFile;
                globalSettings = settings;
                calc(globalJsonFile, globalSettings);
            }
        });
    };

    this.calc = function(jsonFile, settings){
        //arrangeData() //does different depending on indata format
        //matchWithFood() //categorize the data based on strings within different kinds of
        //getClimateData() //hamta klimatdata fran lcadb
        //calcClimate() //analysera vad det innebär
        var foodLib = getFoodDict()["ingredients"]; // Get Library

        foodLib = sortByFoodName(foodLib); //sort by foodname
        ...
        console.log(zeroEmission);

        return unitList;
    };

    this.arrangeData = function (jsonFile, setting) {
    ...
    this.arrangeByUnit = function (rowList) {
        ...
        for (var i = 0; i < len ;i++){
            ...
            if (units[unitName] == undefined){ //if the unit has not been found before
                units[unitName] = this.newUnit(unitName);
            }
    ...
    this.addFoodToUnit = function (units, unitName, food, quantity){
    ...
    this.newUnit = function(unitName){
        var a = {
            "name" : unitName,
            "servings" : -1,
            "carbon" : {"total": 0},
    ...
    this.arrangeDataOld = function (jsonFile, case_setting) { //old function
        /*Takes JSON indata and an integer 1-3 depending on what case is used.
        * Returns javascript object, formatted into: name:, weight:, unit, foodName: none, foodId = 0, co2 = 0  */
        var nameList = jsonFile.data; // Get the array from the json
        var a = nameList[0]["product"]; //just access so we get an exception if it cant
        var b = nameList[0]["weight"];
        switch(3){
            case 1:
                /* Takes in columns: name, weight, unit */
                var len = nameList.length;
                for (i = 0;i < len; i++){
                    nameList[i]["name"] = "None";
                    nameList[i]["id"] = 0;
                    nameList[i]["carbon"] = null;
                    nameList[i]["totCarbon"] = 0;
                }

                //Case 1: Names, Total Weight, and Weight unit in separate arrays

                break;

            case 2:
                //Case 2: Names (Total Weight + Weight unit) in a single column
                break;

            case 3: // Martin & Servera
                //Case 3: Names and Unit Weight in one col, units sold in one col
                //Algorithm extracts the unit from the name and creates a separate arg. unit
                var len = nameList.length;
                for (i = 0;i < len; i++){
                    nameList[i]["name"] = "None";
                    nameList[i]["id"] = 0;
    ...
    this.matchWithFood = function(name) { //testad, verkar fungera
    ...
    this.matchWithFoodFull = function(rowList, foodLib) { //testad, verkar fungera
        //Match each names in foodDict to a name in the sorted array foodLib.
        var len = rowList.length;
        for (var i = 0;i < len;i++){ //for each name
            var words = [rowList[i]["name"]]; //add the full name
            words.concat(rowList[i]["name"].split(" ")); //get the words from the string in array
            var found = false;
            var len = words.length;
            for (var j = 0; j < len && found == false;j++){ //for each word, as long as we have not found a match
                foundFood = inFoodLib(words[j], foodLib);
                if (foundFood == false) { //hittar vi en matchning i foodLib?

                }else{ // if foundfood is not false
                    rowList[i]["food"] = foundFood;
                    found = true;
                }
            }
        }
    ...
    this.inFoodLib = function(word) { //testad
        //takes word and returns true of word is a name in foodLib, otherwise false.
        var foodLib = getFoodDict()["ingredients"];
        var len = globalFoodLib.length;
        for (var i = 0; i < len; i++) {
            if (word.toUpperCase() == globalFoodLib[i]["name"].toUpperCase()) {
                return (foodLib[i]);
            }else{
            }
        }
    ...
    this.sortByFoodName = function (foodLib) {
    ...
    this.getFoodDictGlobal = function (){


        a = getFoodDict();
        var csv = Papa.unparse(a["ingredients"]);

        Papa.parse(csv, { //csv --> json
            header: true,
            dynamicTyping: true,
            encoding : "UTF-8",
            complete: function (jsonFile){
                globalFoodLib = jsonFile.data; //Calls calc
            }
        });
    ...
    main.calc = calc;
    main.arrangeData = arrangeData;
    main.getFoodDict = getFoodDict;
    main.sortByFoodName = sortByFoodName;
    main.inFoodLib = inFoodLib;
    main.matchWithFood = matchWithFood;
    main.newUnit = newUnit;
    main.getFoodDictGlobal = getFoodDictGlobal;
};
main();


//console.log(newCategory(10));

/*

this.getUnitFromEndOfString = function(inStr){
    //gets the unit in the end of the string, or returns none
    var rest = skipTheIntegers(inStr); //Gets the substring after the integers
    var unit = this.getUnitRec(rest, null);
    return unit;

};

this.getUnit = function(inStr){
    ...
    for (var i = len-1;i >= 0;i--){
        c = inStr.charAt(i);
        // om c är g
        //kolla om nästa bokstav är k, m, h eller " "
        // eller om c är l
        //kolla om näst bokstav är d, c, m, eller " "
    }

};


this.getUnitRec = function (inStr, lastChar) {
    // takes string and the last found char if there is one, returns a unit.
    len = inStr.length;
    c = inStr.charAt(len-1);
    if (len == 0) {
        if (lastChar =! null){
            if (lastChar == 'G' || lastChar == 'g'|| lastChar == 'L' || lastChar =='l'){
                return lastChar;
            }
        }else{
            return "";
    ...
    }else if (c == 'g' || c == 'G' || c=='l' ||c == 'L' ){
        return getUnitRec(inStr.substr(0,len-1), c);
    }else if(lastChar != null){ // if there is a lastChar
        if (lastChar == 'g' || lastChar == 'G'){
            if (c == 'K'||c == 'k'||c == 'D'||c == 'd'||c == 'M'||c == 'm'){
                return c.concat("g");
            }else{ //if the first letter fails
                return getUnitRec(inStr.substr(0, len-1),null);
            }
        }else if (lastChar =='l' ||lastChar == 'L' ) {
            if (c == 'C' || c == 'c' || c == 'D' || c == 'd' || c == 'M' || c == 'm') {
                return c.concat("l");
            }else if (c == ' '){
                return ("l");
            } else{
...
this.skipTheIntegers = function(inStr){
    //takes a string, finds sequence of ints and returns the substring after the integers.
    var len = inStr.length;
    var foundInt = false;
    var index = 0;
    for (var i = 0; i < len; i++){
        c = inStr.charAt(i);
        if (!(c == " ") && (!isNaN(c))){ // if c is a number
            foundInt = true;
        }else if (foundInt == true){ //if a number has been found and this is not a number
            index = i; //This is the index where the numbers stop
            break;
        }
    }
    return inStr.substr(index, len);
};
*/